...
### scx-scheds — pluggable BPF schedulers on cachyos/zen (P5)

Add `scx-scheds` to the package set and ship a default-off systemd unit selecting one scheduler (`scx_lavd` for latency-leaning desktop, or `scx_rusty` for throughput). Runtime-switchable, no kernel rebuild — sched-ext is already enabled in both shipped kernels (cachyos default + zen fallback). Surface the on/off + scheduler choice in ATT so users can A/B without editing units. Rationale: the cachyos kernel's biggest lever we're not pulling. Tradeoff/caveat: sched-ext is **only** available on cachyos + zen — if a user later installs `linux-hardened` or `linux-lts` via ATT, the scx unit silently no-ops (acceptable degradation, but breaks strict kernel-agnosticism — see [[feedback-kiro-kernel-agnostic]]). Keep default OFF so the stock boot is unchanged and the kernel-agnostic baseline holds; on = explicit user opt-in. Verification hook: kiro-audit reports active scheduler (`cat /sys/kernel/sched_ext/state`) as INFO, not PASS/FAIL.
