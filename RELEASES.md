...
**Why a new ISO:** Kiro now boots the kernel it says it will, no matter which pairing shipped that month — plus a cleaner default kernel choice and a couple of leftover rough edges gone.
...
**Why a new ISO:** two months of work in one release — the **Arch Linux Tweak Tool** can now install **all 15 Kiro Wayland sessions** (they are added after you install, not baked into the ISO), its **Btrfs page** is in the sidebar on every machine, and the **Calamares installer** picks up a batch of real-hardware fixes.
- **30 desktops on demand, from one ISO:** the **Arch Linux Tweak Tool** now installs **15 X11 desktops** (6 full desktop environments + 9 tiling window managers) and **15 Wayland sessions** — the number the installer slideshow now quotes. Two new X11 tilers joined the list: **dusk** (a dwm fork) and **herbstluftwm**, and both are also selectable editions in the **Kiro ISO Builder**, so you can bake either straight into an ISO you roll yourself.
- **ATT's Wayland picker is finished — every edition installs:** ATT's **Desktop - Wayland** page went from "only Hyprland is selectable" to all 15 curated editions installable: **Hyprland**, **Hyprland Noctalia**, **Hyprland Noctura**, **Hyprland Dank Material Shell**, **Ohmyniri**, **Niri Noctalia**, **Niri Dank Material Shell**, **Mango**, **Miracle**, **River**, **Wayfire**, **Labwc**, **Dwl**, **Sway** and **Scroll**. Each row installs its Kiro config package and seeds that edition's config into your home; removing one takes the config package only, never your `~/.config`.
- **The picker tells you what will go wrong before you click:** picking two editions built on different Quickshell shells (Dank Material Shell vs Noctalia) now raises an amber warning naming the clashing pair and greys out **Install selected**, instead of letting pacman fail with an opaque conflict error. Every row also carries a link to the compositor's own wiki or repo, there are **Select all / Deselect all** buttons, and the "you need an extra repo" notice now correctly accepts **nemesis_repo, chaotic-aur or cachyos** rather than insisting on nemesis_repo alone.
- **A predictable keyboard on every Wayland desktop:** all 15 Wayland editions now log in on **US QWERTY** with **Belgian AZERTY** as the second layout and **Alt+Shift** switching between them. Previously the second layout was loaded but had no switch key, so it was unreachable.
- **Logout and lock work on all of them:** **archlinux-logout** now knows every Wayland edition (and the new dusk/herbstluftwm sessions) and shuts the whole session down cleanly — the bar, notification daemon, idle daemon and tray applet are stopped before the compositor, instead of being left running. On one test box that had been left over multiple sessions, six orphaned idle daemons were fighting over the same D-Bus name and made the logout dialog take 10–14 seconds to appear; it is instant now. The **power menu** on the tiling editions was re-synced with the same logic, which fixes the long-standing "logout needs two presses" on the niri sessions — the compositor is now asked to quit cleanly instead of being killed out from under its own user service.
- **What the Btrfs page in ATT does:** if you installed on **btrfs**, this page turns your system into one that snapshots itself. One click installs `snapper`, `snap-pac`, `btrfs-assistant` and `btrfsmaintenance`, takes a baseline snapshot, and from then on every `pacman` transaction gets a before-and-after pair — so a bad update is something you step back out of. Deliberately no hourly timeline snapshots: you get one per change you made, not a stream of them. A **Disable snapshots** button undoes the whole thing and never deletes a snapshot you already have.
- **The Btrfs page in ATT is now shown on every machine, not only on btrfs ones:** it used to be hidden entirely unless your root filesystem was btrfs, so the tab list differed from one install to the next — confusing when you are following a tutorial and the tab on screen is missing on yours. It is now always listed; on a non-btrfs root it opens, names the filesystem you actually have, and disables every button rather than pretending. **This does not turn snapshots on for anyone** — the feature is still opt-in, and still needs btrfs, which is chosen when you install and cannot be switched afterwards.
- **The rollback advice is corrected:** it used to tell you your bootloader was systemd-boot, which stopped being true once Kiro started installing with GRUB as well. It now says plainly that this setup adds no boot-menu snapshot picker, that you roll back from the running system or from the live ISO, and that a GRUB user can add a picker themselves with `grub-btrfs`.
- **Validated on encrypted btrfs:** this release was installed and audited on **GRUB + LUKS + btrfs** — `kiro-audit` reports **146 PASS / 0 WARN / 0 FAIL**, zero failed units, the full Snapper-ready subvolume layout with snapshots already present, and `snap-pac` creating a real pre/post pair around a live pacman transaction. A separate run on real hardware came back with a single finding, which traced to a bug in the audit tool's own microcode check rather than to the ISO, and was fixed the same day.
- **A new Celestial themes page in ATT — 65 GTK themes:** ATT gained a **Celestial themes** page listing all 65 `celestial-*` themes grouped into eight colour families (Blues, Indigos, Purples, Greens, Reds, Oranges, Pinks, Greys), each with a swatch taken from the theme itself. Every Celestial colour now also ships **Plasma theming** — colour scheme, global theme, desktop theme and Aurorae window decoration — so the same colour can dress GTK, Qt/Kvantum and Plasma. The page also offers **celestial-theme-forge**, which builds the theme in *any* colour you pick, with a GTK4 picker and a screen-colour eyedropper. The old "Themes" page is now named **Arc themes**, so both say what they hold.
- **Two installer fixes you would only hit on real hardware:** the EFI partition minimum was raised from **32 MiB to 1 GiB**, so a tiny pre-existing Windows ESP is rejected up front instead of failing on install or on your first kernel update. And microcode is **never downgraded** any more — the installer used to roll Intel machines back to a three-month-old blob.
- **The installer is clearer when something goes wrong:** a failing `bootctl install` reports the real cause instead of dying later on an unrelated traceback, a reserved system account name is refused on the users page with a message rather than failing halfway through the install, and a display too small for the installer warns instead of blocking. **ext4, xfs and f2fs** now get proper per-filesystem mount options, and the sidebar's current-step highlight is visible again on Calamares 3.4.
- **The keybindings cheatsheet finds every desktop:** `Super+Ctrl+S` now resolves the right cheatsheet on all four Hyprland editions (each runs the same process but its own config folder), on niri, wayfire, sway, river, labwc, dwl, mango, scroll and miracle-wm, and on the new dusk/herbstluftwm sessions. On X11 tiling window managers the window now gets a border, keyboard focus, a working search field and a working **Esc** — it previously opened as an unmanaged window that ignored every keypress.
- **Fish Tweak Tool catches up with fish 4.x:** the Themes tab rendered every swatch black since fish stopped shipping its built-in themes as files; colours are now read from fish itself. Starship preset previews no longer go blank after the first launch. **Hydro** and **Pure** gained inline controls for prompt symbol and colours (with a glyph quick-pick), and **Tide** gets a button that runs its own `tide configure` wizard.
- **A transparent chadwm bar:** **ohmychadwm** gained real bar and border opacity knobs (`THEME_BAROPACITY`, default 85) — they were documented but had no code behind them — and the bar's menu launcher now draws at its own size instead of sitting visibly smaller than the tags beside it. All 45 themes were rebuilt against the change.
- **`ll` works again in fish:** the bundled `ll` alias errored out under recent `eza` versions, which started reading the `h` in `-alFh` as a value for `--classify`. Reordered so it parses under both `eza` and GNU `ls`.
- **A new `kiro-xlibre-swap` tool:** switches a running system between stock **X.Org** and **XLibre** — a drop-in fork of X.Org — and back again, from its own signed repository, with a rollback.
- **Smaller things you'll notice:** `kiro-report` now includes the `kiro-audit` findings, so a support paste shows what is actually wrong with the install; `skell` works as a short alias for `kiro-skell`; the **Hyprland Tweak Tool** lets you knowingly override its "set up snapshots first" gate with an **Install anyway** escape hatch; and the **Kiro ISO Builder** reveals a per-edition extras page the moment you tick that edition instead of on the next launch.
- **Packages:** no additions or removals on the default medium — **`dusk`** and **`hlwm`** join the bakeable edition list, and the release is carried by updates across the tweak tools, `archlinux-logout`, `kiro-keybindings`, the Calamares configuration, `kiro-system-files`, the Wayland edition packages and the new `celestial-*` theme family.
